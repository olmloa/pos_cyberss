...
class CheckLanguages extends Command
{
    ...
    protected $signature = 'app:check-languages';
    ...
    public function __construct(GoogleTranslate $googleTranslate)
    {
        ...
        self::$locales = collect(json_decode(File::get(lang_path('languages.json')))->available)->pluck('value')->all();
    }
    ...
    {
        $path = [app_path(), resource_path('views'), resource_path('js'), base_path('modules/Shop')];
        ...
        $finder = new Finder;
        $finder->in($path)->exclude(['vendor', 'storage'])->files()->name(['*.js', '*.php', '*.vue']);

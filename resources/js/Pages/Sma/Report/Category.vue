...
<template>
  <Head>
    <title>{{ $t('{x} Report', { x: $t('Categories') }) }}</title>
  </Head>
  <Header>
    {{ $t('{x} Report', { x: $t('Categories') }) }}
    <template #subheading>
      ...
      >
        ...
        <span v-if="filters.categories && filters.categories.length > 0">
          {{ $t('Categories Count') }}: {{ filters.categories.length }}, {{ $t('Categories Ids') }}: {{ filters.categories.join(', ') }}
        </span>
      </div>
    </template>
    <template #menu>
  ...
  >
    <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 bg-white px-4 py-6 sm:px-6 xl:px-8 dark:bg-gray-900">
      <dt class="text-mute text-sm leading-6 font-medium">{{ $t('Categories') }}</dt>

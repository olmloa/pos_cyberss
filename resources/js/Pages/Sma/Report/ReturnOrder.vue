...
import QuickView from '@/Pages/Sma/Order/ReturnOrder/QuickView.vue';
import { Button, Loading, Pagination } from '@/Components/Common';

defineOptions({ layout: AdminLayout });
defineProps(['pagination', 'sale_totals', 'purchase_totals', 'stores', 'users']);
...
<template>
  <Head>
    <title>{{ $t('{x} Report', { x: $t('Return Order') }) }}</title>
  </Head>
  <Header>
    {{ $t('{x} Report', { x: $t('Return Order') }) }}
    <template #subheading>
      ...
      >
        ...
        <span v-if="filters.customer_id">
          {{ $t('Customer') }}: {{ pagination.data[0]?.customer?.company || filters.customer_id }}
        </span>
        <span v-if="filters.supplier_id">
          {{ $t('Supplier') }}: {{ pagination.data[0]?.supplier?.company || filters.supplier_id }}
        </span>
        <span v-if="filters.store_id"> {{ $t('Store') }}: {{ pagination.data[0]?.store?.name || filters.store_id }} </span>
  ...
  <dl
    v-if="sale_totals && purchase_totals"
  ...
  >
    <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 bg-white px-4 py-6 sm:px-6 xl:px-8 dark:bg-gray-900">
      <dt class="text-mute text-sm leading-6 font-medium">{{ $t('Sale Return Orders') }}</dt>
      <!-- <dd class="text-xs font-medium">+4.75%</dd> -->
      <dd class="text-focus w-full flex-none text-3xl leading-10 font-medium tracking-tight">
        {{ $currency(sale_totals?.total || 0) }}
        <span class="text-base font-normal">({{ $number(sale_totals?.count || 0, null, { maximumFractionDigits: 0 }) }})</span>
      </dd>
    </div>
    <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 bg-white px-4 py-6 sm:px-6 xl:px-8 dark:bg-gray-900">
      <dt class="text-mute text-sm leading-6 font-medium">{{ $t('Tax (Sale Return)') }}</dt>
      <!-- <dd class="text-xs font-medium text-rose-600">+54.02%</dd> -->
      <dd class="text-focus w-full flex-none text-3xl leading-10 font-medium tracking-tight">
        {{ $currency(sale_totals?.tax || 0) }}
      </dd>
    </div>
    <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 bg-white px-4 py-6 sm:px-6 xl:px-8 dark:bg-gray-900">
      <dt class="text-mute text-sm leading-6 font-medium">{{ $t('Purchase Return Orders') }}</dt>
      <!-- <dd class="text-xs font-medium text-rose-600">+10.18%</dd> -->
      <dd class="text-focus w-full flex-none text-3xl leading-10 font-medium tracking-tight">
        {{ $currency(purchase_totals?.total || 0) }}
        <span class="text-base font-normal">({{ $number(purchase_totals?.count || 0, null, { maximumFractionDigits: 0 }) }})</span>
      </dd>
    </div>
    <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 bg-white px-4 py-6 sm:px-6 xl:px-8 dark:bg-gray-900">
      <dt class="text-mute text-sm leading-6 font-medium">{{ $t('Tax (Purchase Return)') }}</dt>
      <!-- <dd class="text-xs font-medium">-1.39%</dd> -->
      <dd class="text-focus w-full flex-none text-3xl leading-10 font-medium tracking-tight">
        {{ $currency(purchase_totals?.tax || 0) }}
      </dd>
    </div>
  </dl>
  ...
  <div class="relative flex grow flex-col items-stretch justify-stretch self-stretch bg-white px-4 sm:px-6 lg:px-8 dark:bg-gray-800">
    ...
    <Modal :show="form" max-width="2xl" @close="form = false" :overflow="true">
      <Customize
        ...
        :filters="filters"
        @close="form = false"
        :fields="['products', 'customer', 'supplier', 'store', 'user']"
      />
    </Modal>

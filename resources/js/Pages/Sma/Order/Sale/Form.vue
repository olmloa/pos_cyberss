...
import { $extras, calculate_item, check_promotions, discount_keypress, convert_to_base_unit } from '@/Core';
...
async function addItem(item) {
  if (form.items.find(i => i.product_id == item.id)) {
    ...
    const product = await axios.get(route('products.show', { product: item.id, with: 'promotions' })).then(r => r.data);
...
function handleSubmit(e, listing = false) {
  ...
  form
    ...
    .post(props.current?.id ? route('sales.update', props.current.id) : route('sales.store'), {
      forceFormData: true,
      onSuccess: () => {
        resetForm();
        if (listing) {
...
<template>
  <Head>
    <title>{{ current?.id ? $t('Edit {x}', { x: $t('Sale') }) : $t('Add {x}', { x: $t('Sale') }) }}</title>
  </Head>

  <div class="px-0 pt-6 pb-0 sm:px-6 sm:py-8 lg:px-8">
    <FormSection @submitted="handleSubmit">
      <template #title>
        {{ current?.id ? $t('Edit {x}', { x: $t('Sale') }) : $t('Add {x}', { x: $t('Sale') }) }}
      </template>
      ...
      <template #description>
        <div class="block w-full sm:flex sm:items-start sm:justify-between lg:block">
          ...
          <div class="me-3 mt-6 sm:mt-0 lg:mt-6">
            <Link class="link" :href="route('sales.index')">{{ $t('List {x}', { x: $t('Sales') }) }}</Link>
          </div>
        </div>
      </template>
      ...
      <template #form>
        ...
        <!-- Due Date -->
        <div class="col-span-6 sm:col-span-3">
          <DateInput id="due_date" @change="saveForm" :label="$t('Due Date')" v-model="form.due_date" :error="form.errors.due_date" />
        </div>
        ...
        <div class="col-span-full">
          <ProductSearch model="sale" @add-item="addItem" />

          <div v-if="form.items && form.items.length" class="overflow-x-auto">
            <h4 class="mt-6 pb-2 text-lg font-bold">{{ $t('Sale Items') }}</h4>
    ...
    <div v-if="selectedItem">
      ...
      <div>
        ...
        <div class="col-span-full mb-6 px-6">
          <Textarea id="item-comment" :label="$t('Comment')" v-model="selectedItem.comment" />
        </div>

...
defineProps(['pagination', 'gift_card']);
...
<template>
  <Head>
    <title>{{ $t('Gift Card Logs') }}</title>
  </Head>
  <Header>
    {{ $t('Gift Card Logs') }} ({{ gift_card.number }})
    <template #subheading> {{ $t('Please review the data below') }} </template>
    <template #menu>
      <div class="flex items-center justify-center gap-4">
        <Link class="btn-primary" :href="route('gift_cards.index')">
          {{ $t('List {x}', { x: $t('Gift Cards') }) }}
        </Link>
      </div>
    </template>
  </Header>
          ...
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead>
              <tr>
                ...
                <th scope="col" class="text-focus py-3.5 ps-3 pe-6 text-start text-sm font-semibold">
                  <button
                    type="button"
                    class="flex items-center gap-2 whitespace-nowrap"
            ...
            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
              <template v-if="pagination && pagination.data && pagination.data.length">
                <tr v-for="row in pagination.data" :key="row.id" :class="row.deleted_at ? 'bg-red-100 dark:bg-red-950' : ''">
                  ...
                  <td class="py-4 ps-3 pe-6 text-end text-sm whitespace-nowrap">{{ $currency(row.value) }}</td>
                </tr>
              </template>
              <tr v-else>
                <td colspan="100%">

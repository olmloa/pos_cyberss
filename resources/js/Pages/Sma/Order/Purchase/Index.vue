...
defineProps(['pagination', 'custom_fields', 'payment_fields']);
...
function editRow(row) {
  router.visit(route('purchases.edit', { purchase: row.id }));
}

function emailRow(row) {
  router.post(route('email.purchase', { purchase: row.id }));
}

function deleteRow(row) {
  deleting.value = true;
  router.delete(route('purchases.destroy', row.id), {
    preserveScroll: true,
...
<template>
  ...
  <Header>
    ...
    <template #menu>
      <div class="flex items-center justify-center gap-4">
        <Button v-if="$can('create-purchases')" :href="route('purchases.create')">
          {{ $t('Add {x}', { x: $t('Purchase') }) }}
        </Button>
  ...
  <div class="relative flex grow flex-col items-stretch justify-stretch self-stretch bg-white px-4 sm:px-6 lg:px-8 dark:bg-gray-800">
            ...
            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
              <template v-if="pagination && pagination.data && pagination.data.length">
                <tr
                  ...
                  :key="row.id"
                  :class="{
                    'bg-red-50 dark:bg-red-800/40': row.deleted_at || row.return_orders_count > 0,
                    'bg-green-50 dark:bg-green-800/40': !row.return_orders_count && row.paid >= row.grand_total,
                    'bg-yellow-50 dark:bg-yellow-800/40': !row.return_orders_count && row.paid < row.grand_total,
                  }"
                >
                  <td
                    @click="viewRow(row)"
                  ...
                  <td v-if="$can('create-payments')" class="text-right text-sm whitespace-nowrap">
                    <button
                      v-if="Number(row.paid) >= Number(row.grand_total)"
                      type="button"
                      @click="viewRow(row)"
                  ...
                  <td
                    ...
                    :class="{
                      'bg-red-100! dark:bg-red-950!': row.deleted_at || row.return_orders_count > 0,
                      'bg-green-100! dark:bg-green-950!': !row.return_orders_count && row.paid >= row.grand_total,
                      'bg-yellow-100! dark:bg-yellow-950!': !row.return_orders_count && row.paid < row.grand_total,
                    }"
                  >
                    ...
                    <div class="text-mute flex items-center justify-end gap-4">
                      ...
                      <button v-if="$can('email-purchases')" type="button" class="link" @click="emailRow(row)">
    ...
    <Modal :show="payment" max-width="2xl" @close="payment = false">
      <PaymentForm :purchase="current" :customer="current?.supplier" :custom_fields="payment_fields" @close="payment = false" />
    </Modal>
  </div>
</template>

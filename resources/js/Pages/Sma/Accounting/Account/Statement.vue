...
defineProps(['pagination', 'account']);
...
<template>
  <Head>
    <title>{{ $t('Account Statement') }}</title>
  </Head>
  <Header>
    {{ $t('Account Statement') }} ({{ account.title }}) ({{ $t('Balance') }}: {{ $currency(account.balance) }})
    <template #subheading> {{ $t('Please review the data below') }} </template>
    <template #menu>
      <div class="flex items-center justify-center gap-4">
        <Link class="btn-primary" :href="route('accounts.index')">
          {{ $t('List {x}', { x: $t('Accounts') }) }}
        </Link>
      </div>
    </template>
  </Header>

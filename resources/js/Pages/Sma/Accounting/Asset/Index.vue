...
defineProps(['pagination', 'categories', 'conditions']);
...
function deleteRow(row) {
  deleting.value = true;
  router.delete(route('assets.destroy', row.id), {
    preserveScroll: true,
...
<template>
  ...
  <Header>
    ...
    <template #menu>
      <div class="flex items-center justify-center gap-4">
        <Button v-if="$can('create-assets')" type="button" @click="add = true">
          {{ $t('Add {x}', { x: $t('Asset') }) }}
        </Button>
  ...
  <div class="relative flex grow flex-col items-stretch justify-stretch self-stretch bg-white px-4 sm:px-6 lg:px-8 dark:bg-gray-800">
          ...
          <table class="fixed-actions min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead>
              <tr>
                ...
                <th scope="col" class="text-focus px-3 py-3.5 text-start text-sm font-semibold">{{ $t('Category') }}</th>
                <th scope="col" class="text-focus px-3 py-3.5 text-start text-sm font-semibold">{{ $t('Serial Number') }}</th>
                <th scope="col" class="text-focus px-3 py-3.5 text-start text-sm font-semibold">
                  <button
                    type="button"
                    class="flex items-center gap-2 whitespace-nowrap"
                    @click="sortBy(filters?.sort == 'purchase_cost:asc' ? 'purchase_cost:desc' : 'purchase_cost:asc')"
                  >
                    {{ $t('Purchase Cost') }}
                    <Icon
                      size="size-3 text-mute"
                      v-if="filters?.sort?.startsWith('purchase_cost:')"
                      :name="filters.sort == 'purchase_cost:desc' ? 'c-up' : 'c-down'"
                    />
                  </button>
                </th>
                <th scope="col" class="text-focus px-3 py-3.5 text-start text-sm font-semibold">{{ $t('Condition') }}</th>
                <th scope="col" class="text-focus px-3 py-3.5 text-center text-sm font-semibold">{{ $t('Active') }}</th>
            ...
            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
              <template v-if="pagination && pagination.data && pagination.data.length">
                <tr v-for="row in pagination.data" :key="row.id">
                  ...
                  <td class="px-3 py-4 text-sm whitespace-nowrap">{{ row.serial_number }}</td>
                  <td class="px-3 py-4 text-sm whitespace-nowrap">{{ $number(row.purchase_cost) }}</td>
                  <td class="px-3 py-4 text-sm whitespace-nowrap">{{ row.condition }}</td>
                  <td class="w-16 px-3 py-4 text-center text-sm whitespace-nowrap">
                    <span v-html="$boolean(row.active)" class="-mb-1 inline-block size-5"></span>
                  </td>
    ...
    <Modal :show="add" :backdrop="false" max-width="2xl" :closeable="true" @close="hideForm">
      <Form :current="current" :categories="categories" :conditions="conditions" @close="hideForm" @done="hideForm" />
    </Modal>
  </div>
</template>

...
defineProps(['pagination', 'customer']);
...
<template>
  <Head>
    <title>{{ $t('Customer Statement') }}</title>
  </Head>
  <Header>
    {{ $t('Customer Statement') }} ({{ customer.company || customer.name }})
    <template v-if="customer.balance < 0">({{ $t('Adv') }}: {{ $currency(0 - customer.balance) }})</template>
    <template v-else>({{ $t('Due') }}: {{ $currency(customer.balance) }})</template>
    <template #subheading> {{ $t('Please review the data below') }} </template>
    <template #menu>
      <div class="flex items-center justify-center gap-4">
        <Link class="btn-primary" :href="route('customers.index')">
          {{ $t('List {x}', { x: $t('Customers') }) }}
        </Link>
      </div>
    </template>
  </Header>

...
function submit() {
  ...
  // this.$inertia.post(route('customers.import.save'), data);
  form.post(route('customers.import.save'), { preserveScroll: true });
}
</script>

<template>
  <Head :title="$t('Import {x}', { x: t('Customers') })" />
  <div class="px-0 pt-6 pb-0 sm:px-6 sm:py-8 lg:px-8">
    <FormSection @submitted="submit">
      <template #title>{{ $t('Import {x}', { x: t('Customers') }) }}</template>
      <template #description>
        <div class="block w-full sm:flex sm:items-start sm:justify-between lg:block">
          ...
          <div class="me-3 mt-6 flex gap-x-8 gap-y-4 sm:mt-0 lg:mt-6">
            <Link class="link" :href="route('customers.index')">{{ $t('List {x}', { x: $t('Customers') }) }}</Link>
            <a
              class="link"
              v-if="route().has('customers.export') && $can('export-customers')"
              :href="route('customers.export', { template: true })"
            >
            ...
            <div class="space-y-1 text-center">
              ...
              <div class="text-xs text-gray-600 dark:text-gray-400">
                <div>
                  {{
                    t(
                      'Excel file should have name, phone, email, company, due_limit, price_group, customer_group, lot_no, street, address_line_1, address_line_2, city, postal_code, state and country columns.'
                    )
                  }}
                </div>
                <div class="mt-2">
                  {{ $t('System will update the customer if it has the same name, phone and email otherwise new customer will be added.') }}
                </div>
              </div>

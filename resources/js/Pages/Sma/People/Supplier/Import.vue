...
function submit() {
  ...
  // this.$inertia.post(route('suppliers.import.save'), data);
  form.post(route('suppliers.import.save'), { preserveScroll: true });
}
</script>

<template>
  <Head :title="$t('Import {x}', { x: t('Suppliers') })" />
  <div class="px-0 pt-6 pb-0 sm:px-6 sm:py-8 lg:px-8">
    <FormSection @submitted="submit">
      <template #title>{{ $t('Import {x}', { x: t('Suppliers') }) }}</template>
      <template #description>
        <div class="block w-full sm:flex sm:items-start sm:justify-between lg:block">
          ...
          <div class="me-3 mt-6 flex gap-x-8 gap-y-4 sm:mt-0 lg:mt-6">
            <Link class="link" :href="route('suppliers.index')">{{ $t('List {x}', { x: $t('Suppliers') }) }}</Link>
            <a
              class="link"
              v-if="route().has('suppliers.export') && $can('export-suppliers')"
              :href="route('suppliers.export', { template: true })"
            >
          ...
          >
            <div class="space-y-1 text-center">
              ...
              <div class="text-xs text-gray-600 dark:text-gray-400">
                ...
                <div class="mt-2">
                  {{ $t('System will update the supplier if it has the same name, phone and email otherwise new supplier will be added.') }}
                </div>
              </div>

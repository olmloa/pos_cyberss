...
function deleteRow(row) {
  deleting.value = true;
  router.delete(route('service-types.destroy', row.id), {
    preserveScroll: true,
...
<template>
  <Head>
    <title>{{ $t('Service Types') }}</title>
  </Head>
  <Header>
    {{ $t('Service Types') }}
    <template #subheading> {{ $t('Manage repair service types') }} </template>
    <template #menu>
      <div class="flex items-center justify-center gap-4">
        <Button v-if="$can('create-service-types')" @click="form = true">
          {{ $t('Add {x}', { x: $t('Service Type') }) }}
        </Button>
      </div>
    </template>
  </Header>
              ...
              <tr>
                ...
                <th scope="col" class="text-focus px-3 py-3.5 text-center text-sm font-semibold">
                  <button
                    ...
                    @click="sortBy(filters?.sort == 'sort_order:asc' ? 'sort_order:desc' : 'sort_order:asc')"
                  >
                    {{ $t('Sort Order') }}
                    <Icon
                      size="size-3 text-mute"
                      v-if="filters?.sort?.startsWith('sort_order:')"
                      :name="filters.sort == 'sort_order:desc' ? 'c-up' : 'c-down'"
                    />
                  </button>
                </th>
                <th scope="col" class="text-focus px-3 py-3.5 text-center text-sm font-semibold">

...
function editRow(row) {
  router.visit(route('transfers.edit', { transfer: row.id }));
}

function emailRow(row) {
  router.post(route('email.transfer', { transfer: row.id }));
}
  ...
  <Header>
    ...
    <template #menu>
      <div class="flex items-center justify-center gap-4">
        <Button v-if="$can('create-transfers')" :href="route('transfers.create')">

...
function deleteRow(row) {
  deleting.value = true;
  router.delete(route('brands.destroy', row.id), {
    preserveScroll: true,
  ...
  <Header>
    ...
    <template #menu>
      <div class="flex items-center justify-center gap-4">
        <Button v-if="$can('create-brands')" type="button" @click="add = true">
          {{ $t('Add {x}', { x: $t('Brand') }) }}
        </Button>
        <Dropdown align="right" width="40" :auto-close="false">
          ...
          <template #content>
            <div>
              ...
              <DropdownLink v-if="route().has('brands.import')" :href="route('brands.import')">
                {{ $t('Import {x}', { x: $t('Brands') }) }}
              </DropdownLink>
              <DropdownLink as="a" v-if="route().has('brands.export')" :href="route('brands.export')">
                {{ $t('Export {x}', { x: $t('Brands') }) }}
              </DropdownLink>
            </div>
          </template>
        </Dropdown>

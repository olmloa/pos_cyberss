...
function editRow(row) {
  router.visit(route('adjustments.edit', { adjustment: row.id }));
}

function deleteRow(row) {
  deleting.value = true;
  router.delete(route('adjustments.destroy', row.id), {
    preserveScroll: true,
  ...
  <Header>
    ...
    <template #menu>
      <div class="flex items-center justify-center gap-4">
        <Button v-if="$can('create-adjustments')" :href="route('adjustments.create')">
          {{ $t('Add {x}', { x: $t('Adjustment') }) }}
        </Button>

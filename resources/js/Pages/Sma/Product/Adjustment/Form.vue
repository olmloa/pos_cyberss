...
import { Attachments, AutoComplete, CustomFields, FileInput, Input, LoadingButton, ProductSearch, Textarea } from '@/Components/Common';

defineOptions({ layout: AdminLayout });
const props = defineProps(['current', 'custom_fields']);
...
function handleSubmit(e, listing = false) {
  ...
  form
    ...
    .post(props.current?.id ? route('adjustments.update', props.current.id) : route('adjustments.store'), {
      onSuccess: () => {
        resetForm();
        if (listing) {
...
<template>
  <Head>
    <title>{{ current?.id ? $t('Edit {x}', { x: $t('Adjustment') }) : $t('Add {x}', { x: $t('Adjustment') }) }}</title>
  </Head>

  <div class="px-0 pt-6 pb-0 sm:px-6 sm:py-8 lg:px-8">
    <FormSection @submitted="handleSubmit">
      <template #title>
        {{ current?.id ? $t('Edit {x}', { x: $t('Adjustment') }) : $t('Add {x}', { x: $t('Adjustment') }) }}
      </template>
      ...
      <template #description>
        <div class="block w-full sm:flex sm:items-start sm:justify-between lg:block">
          ...
          <div class="me-3 mt-6 sm:mt-0 lg:mt-6">
            <Link class="link" :href="route('adjustments.index')">{{ $t('List {x}', { x: $t('Adjustments') }) }}</Link>
          </div>
        </div>
      </template>
      ...
      <template #form>
        ...
        <div class="col-span-6 sm:col-span-3">
          <AutoComplete
            ...
            :suggestions="[
              { value: 'Addition', label: $t('Addition/Extra/Found') },
              { value: 'Subtraction', label: $t('Subtraction/Damage/Expired/Lost') },
            ]"
          />
        </div>
        <!-- Reference -->
        ...
        <div class="col-span-full">
          <ProductSearch model="adjustment" :type="form.type" @add-item="addItem" />
          <div v-if="form.items && form.items.length" class="border-b border-gray-200 dark:border-gray-700">
            <h4 class="mt-6 border-b border-gray-200 pb-1 text-lg font-bold dark:border-gray-700">{{ $t('Adjustment Items') }}</h4>
